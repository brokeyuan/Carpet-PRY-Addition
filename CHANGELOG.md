...
- **`realisticPlayerScale` 重力联动（下落速度）**：下落加速度随体型平方根缩放（×√scale，+0.3 保底）——大体型下落明显更快（scale 4→2×、16→4×），缩小更飘逸（0.25→0.5×）；与跳跃 √ 曲线配套后大体型相对起跳高度与原版一致（若 ≥1 用线性，16 倍体型终端速度约 62 格/tick 会失控）。创造飞行/鞘翅滑翔时原版不施加重力，不受影响
