...
| `realisticPlayerScale` | boolean | `false` | Player physics scale with size (`minecraft:scale` attribute): movement/flying speed scale with size (√ curve when shrinking, gentler, with floor), step height, interaction ranges and safe fall distance scale linearly (with floors), jump and gravity scale with the square root (larger players fall faster, smaller ones more floaty), Requires `playerScale`, Minecraft 1.21.5+ only |
