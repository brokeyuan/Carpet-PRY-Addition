...
为 Player 注册 `minecraft:scale` 属性，并通过 `/scale set|reset|info` 命令调节玩家体型大小（value 仅要求大于 0，不设上下限；本模组放行 SCALE 属性的任意有限正值，保证命令反馈值与实际生效值一致）。仅 1.21.5+ 版本支持。

| 属性 | 值 |
|------|-----|
| **规则名** | `playerScale` |
| **描述** | 为 Player 注册 minecraft:scale 属性，并添加 /scale set/reset/info 命令（value 仅要求大于 0，不设上下限；超出原版属性范围 0.0625–16 时纯原版客户端的显示仍会被夹紧）。false=关闭命令；self=所有人都只能调自己（无论 OP）；true=玩家仅可调自己，管理员可调任意玩家；everyone=所有人可调任意玩家。含客户端 FOV 补偿：配合「更真实的玩家大小变」规则时补偿缩小带来的视野变窄（需客户端安装）。仅 1.21.5+ 版本支持 |
| **类型** | `string` |
| **默认值** | `false` |
...
非管理员玩家执行 `/scale set` 时可设置的最小 scale 值，管理员不受此限制（管理员实际可设范围仅要求 value > 0）。

| 属性 | 值 |
|------|-----|
| **规则名** | `playerScaleMin` |
| **描述** | 非管理员玩家执行 /scale set 时可设置的最小 scale 值，管理员不受此限制 |
| **类型** | `double` |
| **默认值** | `0.01` |
...
非管理员玩家执行 `/scale set` 时可设置的最大 scale 值，管理员不受此限制（管理员实际可设范围仅要求 value > 0）。

| 属性 | 值 |
|------|-----|
| **规则名** | `playerScaleMax` |
| **描述** | 非管理员玩家执行 /scale set 时可设置的最大 scale 值，管理员不受此限制 |
| **类型** | `double` |
| **默认值** | `16.0` |
...
| 摔落安全距离 | 线性 | 0.5× (≥1.5格) |
| 视野（FOV） | 补偿缩小带来的视野变窄，保持观感一致（由 `playerScale` 规则的客户端部分实现，需客户端安装本模组） | — |
...
| **描述** | 体型全方位联动：移动/飞行速度随体型平方根缩放（缩小更平缓，有保底），台阶高度、方块与实体交互距离、摔落安全距离随体型等比缩放（缩小有保底下限确保可玩性），跳跃高度随体型平方根缩放（含保底）。需配合玩家随地大小变规则使用，仅 1.21.5+ 版本支持 |
| **类型** | `boolean` |
| **默认值** | `false` |
| **参考选项** | `false`, `true` |
| **分类** | `PRIMARYUAN`, `SURVIVAL`, `FEATURE` |
